...
void QuadraticProbing::createAccount(std::string id, int count) {
    ...
    if (bankStorage1d[probe].id == "\0" or bankStorage1d[probe].id == "DELETED") {
        ...
        int i = 1;
        while (bankStorage1d[(probe + i*i)%100003].id != "\0" and bankStorage1d[(probe + i*i)%100003].id != "DELETED") {
            i++;
        }
        bankStorage1d[(probe + i*i)%100003] = ac;
    }
}
...
int QuadraticProbing::getBalance(std::string id) {
    ...
    if (bankStorage1d[hashvalue].id == "\0") {
        ...
        if (bankStorage1d[hashvalue].id == id) {
            ...
            while (bankStorage1d[(probe + i*i)%100003].id != "\0") {
                if (bankStorage1d[(probe + i*i)%100003].id == id) {
                    return bankStorage1d[(probe + i*i)%100003].balance;
                }
                else {
                    i++;
                }
...
void QuadraticProbing::addTransaction(std::string id, int count) {
    ...
    if (bankStorage1d[hashvalue].id == id) {
        std::vector<int>::iterator itr1 = QuadraticProbing::find(balances, bankStorage1d[hashvalue].balance);
        balances.erase(itr1);
    ...
    while (bankStorage1d[(probe + i*i)%100003].id != "\0") {
        if (bankStorage1d[(probe + i*i)%100003].id == id) {
            std::vector<int>::iterator itr1 = QuadraticProbing::find(balances, bankStorage1d[(probe + i*i)%100003].balance);
            balances.erase(itr1);

            bankStorage1d[(probe + i*i)%100003].balance += count;

            std::vector<int>::iterator itr2 = std::lower_bound(balances.begin(), balances.end(), bankStorage1d[(probe + i*i)%100003].balance);
            balances.insert(itr2, bankStorage1d[(probe + i*i)%100003].balance);
            return;
        }
        else {
            i++;
        ...
        if (bankStorage1d[hashvalue].id == id) {
            ...
            while (bankStorage1d[(probe + i*i)%100003].id != "\0") {
                if (bankStorage1d[(probe + i*i)%100003].id == id) {
                    ...
                    bankStorage1d[(probe + i*i)%100003].balance = 0;
                    bankStorage1d[(probe + i*i)%100003].id = "DELETED";
